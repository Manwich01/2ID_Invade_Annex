...
{	
	...
	_volume = 	[_this, 5, 10] call Option_Param;
	_pitch = 	[_this, 6, 1.0] call Option_Param;
	_distance = [_this, 7, 30] call Option_Param;
	_inside = 	[_this, 8, false] call Option_Param;
...
Sound_Once3D =
{
	...
	_volume = [_this, 2, 10] call Option_Param;
	_pitch = [_this, 3, 1.0] call Option_Param;
	_distance = [_this, 4, 30] call Option_Param;
	_inside = [_this, 5, false] call Option_Param;
	_soundposition = getPos _soundsourceobject;
	playSound3D [_soundfile, _soundsourceobject, _inside, _soundposition, _volume, _pitch, _distance];
}
