...
Option_Param = 
{
	private["_passed_through", "_parameter_num", "_default_value", "_ret"];
	_passed_through = _this select 0;
	_parameter_num = _this select 1;
	_default_value = _this select 2;
	if (count _passed_through > _parameter_num) then 
	{
		_ret = (_passed_through select _parameter_num);
	} else 
	{
		_ret = _default_value;
	};
